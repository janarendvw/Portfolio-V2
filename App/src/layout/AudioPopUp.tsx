  ...
  return (
    ...
    >
      <div className="w-1/2 max-w-lg rounded-box p-4 bg-base-100 flex items-center justify-evenly gap-2 flex-col">
        <p className="text-xl">Hi there! This site uses audio to enhance the experience. Would you like to have this enabled by default?</p> <br/>
        <p className="opacity-60 italic">P.S. You can always change the volume settings in the footer.</p>
        <span className="flex w-full gap-4 items-center justify-evenly px-2 pt-8">
        <button className="btn w-1/2 flex gap-2" onClick={() => {
setPopup("hidden");
localStorage.setItem("audio", "false");
        }}>
          <VolumeX /> Disable sound
        </button>
        <button className="btn btn-primary w-1/2 flex gap-2" onClick={() => {
          setPopup("hidden");
